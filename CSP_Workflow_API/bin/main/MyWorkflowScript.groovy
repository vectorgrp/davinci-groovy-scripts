...
daVinci {
		...
		code {
			...
			workflow.updateProject(dpaProject) {
				...
				updateSettings{
					...
					def pythonScript = "D:/PES_8/FOSS/KnowledgeBase/CSP_Workflow_API/input/helloWorld.py"
	...
	/*
     * Task: GenStepTask
     * Type: DV_CUSTOM_WORKFLOW_STEP
     * -------------------------------------------------------------------------------------------------------
     * Task can be added as a custom workflow step and will be executed (depending on project settings) after generation
     * -------------------------------------------------------------------------------------------------------
     */
	scriptTask("GenStepTask", DV_CUSTOM_WORKFLOW_STEP){
		taskDescription 'Task prints some text in the console and can be executed after generation is finished'

		taskHelp '''Task can be added as a custom workflow step and will be executed (depending on project settings) after generation
        '''

		code{

			println "AI Custom workflow step was successfully executed"
		}
	}
}